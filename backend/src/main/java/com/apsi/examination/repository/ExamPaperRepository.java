...
public interface ExamPaperRepository extends JpaRepository<ExamPaperEntity, Long> {
    ...
    @Query(value = "SELECT ep.* FROM exampaper ep JOIN examtemplate et ON ep.exam_id = et.id WHERE (ep.status = 'WRITING' OR ep.status = 'UNRESOLVED') AND (et.date_time_end < :currentDateTime)", nativeQuery = true)
